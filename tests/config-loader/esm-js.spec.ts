...
const fixturesPath = getDirname(import.meta.url, 'fixtures/esm-js');

describe('ESM-JS config loader tests', () => {
  describe('loadConfig', () => {
    it('should load a valid .js config file', async () => {
      const loadedCfg = await loadConfig<{
        cake: string;
      }>(join(fixturesPath, 'valid.config.js'), loaderOptions);
    ...
    it('should load a config with `defineConfig` and type parameters', async () => {
      const loadedCfg = await loadConfig<{
        cake: string;
      }>(join(fixturesPath, 'defineConfig.config.js'), loaderOptions);
      expect(typeof loadedCfg?.config).toBe('object');
      expect(loadedCfg?.config.cake).toBe('a lie');
    });
    ...
    it('should find and load config file in nested directory', async () => {
      const searchResult = await loadConfig<{
        cake: string;
      }>(join(fixturesPath, 'nested/valid.config.js'), loaderOptions);
    ...
    it('should handle non-existent file gracefully', async () => {
      const loadedCfg = await loadConfig(
        join(fixturesPath, 'non-existent.config.js'),
        loaderOptions
      );
  ...
  describe('searchConfig', () => {
    it('should search and load a valid .js config file', async () => {
    ...
    it('should search and load a `defineConfig` .js config file', async () => {
      const loadedCfg = await searchConfig<{
        cake: string;
